...
export function getModelSlot(model: Pick<ModelMappingEntry, "name" | "slot">) {
  return model.slot?.trim() || DEFAULT_CLAUDE_SLOTS[0];
}
...
export function normalizeModelMappingConfig(config: ModelMappingConfig): ModelMappingConfig {
  ...
  for (const provider of config.providers) {
    for (const model of provider.models) {
      const trimmed = model.slot?.trim() ?? "";
      if (!trimmed || isAutoGeneratedSlot(trimmed, model.name)) continue;
      reservedSlots.add(normalizeExplicitSlot(trimmed));
    }
  }
    ...
    providers: config.providers.map((provider) => ({
      ...
      models: provider.models.map((model) => {
        ...
        return {
          ...
          slot: normalizeModelSlot(model.slot, model.name, allocateAutoSlot),
