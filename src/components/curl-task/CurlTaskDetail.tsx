...
import { ArrowLeft, Save, Play, Trash2, Loader2, Terminal, CheckCircle2, Bug } from "lucide-react"
import type { CurlTask, ParsedCurl, RequestDebugInfo } from "@/types"
import { loadCurlTasks, saveCurlTask, deleteCurlTask, executeCurlRaw, parseCurlCommand } from "@/api"
...
export default function CurlTaskDetail({ taskId, mode, onBack }: CurlTaskDetailProps) {
	...
	async function handleSave() {
		...
		try {
			...
			const task: CurlTask = {
				...
				label: label.trim(),
				curl,
				parsedCurl: parsed || { url: "", method: "GET", headers: {} },
				selectedFields: selectedFields ?? [],
				lastResult: lastResult ?? null,
				lastRunAt: lastResult ? Date.now() : null,
				createdAt: isNew ? Date.now() : Date.now(),
				updatedAt: Date.now(),
			}
			console.log("[save] task:", JSON.stringify(task, null, 2).slice(0, 500))
	...
	async function handleRun() {
		...
		try {
			setRunning(true)
			const result = await executeCurlRaw(curl)

			if (result.request_debug) {
				setRequestDebug(result.request_debug)
			}
	...
	async function handleFieldConfirm(fields: string[]) {
		...
		toast(`已选择 ${fields.length} 个字段，请保存任务`)
	}
		...
		<div className='flex h-full flex-col'>
			<div className='flex items-center justify-between border-b border-border-subtle px-6 py-3'>
				<div className='flex items-center gap-2'>
					...
					<h2 className='text-sm font-semibold text-text-heading'>{isNew ? "添加任务" : "编辑任务"}</h2>
				</div>
				<div className='flex items-center gap-2'>
					{!isNew && (
						<button
			...
			<div className='flex-1 overflow-y-auto p-4'>
				<div className='mx-auto max-w-2xl space-y-4'>
					...
					<div className='rounded-xl border border-border-subtle bg-surface-base p-4'>
						<h3 className='mb-3 text-xs font-semibold uppercase tracking-wider text-text-muted'>基本信息</h3>
					...
					{/* cURL 配置 */}
					<div className='rounded-xl border border-border-subtle bg-surface-base p-4'>
						<h3 className='mb-3 text-xs font-semibold uppercase tracking-wider text-text-muted'>cURL 配置</h3>
						<div className='space-y-3'>
							<div>
							...
							<div className='flex gap-2'>
								<button
									onClick={handleParse}
									disabled={!curl.trim()}
									className='flex-1 rounded-lg border border-border-subtle px-3 py-2 text-xs font-medium text-text-body transition-colors hover:bg-surface-hover disabled:opacity-50'>
									解析 cURL
								</button>
								<button
									onClick={handleRun}
									disabled={running || !curl.trim()}
									className='flex items-center justify-center gap-1.5 rounded-lg bg-emerald-500 px-4 py-2 text-xs font-medium text-white transition-colors hover:bg-emerald-600 disabled:opacity-50'>
							...
							{requestDebug && (
								<div className='rounded-lg border border-amber-500/30 bg-amber-500/10 p-3'>
									...
									<div className='space-y-1 text-[10px] text-text-body'>
										<div className='font-mono text-amber-300'>
											{requestDebug.method} {requestDebug.url}
										</div>
										{requestDebug.headers.map(([key, value]) => (
											<div key={key} className='flex gap-1'>
												<span className='shrink-0 text-text-muted'>{key}:</span>
												<span className={key.toLowerCase() === "cookie" ? "font-mono text-amber-200" : "font-mono"}>
					...
					{selectedFields.length > 0 && (
						<div className='rounded-xl border border-border-subtle bg-surface-base p-4'>
							<div className='mb-3 flex items-center justify-between'>
								<h3 className='text-xs font-semibold uppercase tracking-wider text-text-muted'>已选字段</h3>
								<button
									onClick={() => {
										if (lastResult) {
					...
					{lastResult && selectedFields.length > 0 && (
						<div className='rounded-xl border border-border-subtle bg-surface-base p-4'>
							<div className='mb-3 flex items-center gap-1.5 text-[11px] font-medium text-text-muted'>
