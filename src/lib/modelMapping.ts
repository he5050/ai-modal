...
export const MODEL_MAPPING_TARGET_PROTOCOLS = ["claude", "openai-chat", "openai-responses", "gemini"] as const
...
function resolveSupportedProtocols(
	model: Pick<ModelMappingEntry, "supported_protocols" | "source_protocol" | "protocol">,
) {
	const values = normalizeProtocolList([...(model.supported_protocols ?? []), model.source_protocol, model.protocol])
	return values
}
...
function normalizeExplicitSlot(slot: string) {
	...
	const matchedSlot = DEFAULT_CLAUDE_SLOTS.find((s) => s.toLowerCase().replace(/^anthropic\//i, "") === normalizedSlot)
	if (matchedSlot) {
		return matchedSlot
	}
	// 否则规范化槽位名称
...
export function normalizeModelSlot(slot: string | undefined | null, modelName: string, _nextAutoSlot: () => string) {
	const trimmed = slot?.trim() ?? ""
	// 不再自动填充槽位，保持用户设置或为空
...
function normalizeModelDisplayName(displayName: string | undefined | null, providerName: string, modelName: string) {
...
export function providerToMappingProvider(provider: Provider, selectedModels?: string[]): ModelMappingProvider {
