...
export function makeModelSlot(providerName: string, order: number) {
  return `anthropic/claude-claude-${sanitizeRouteSegment(providerName)}-${order}`;
}
...
export function normalizeModelSlot(
  ...
  if (!trimmed || isAutoGeneratedSlot(trimmed, modelName)) {
    return makeModelSlot(providerName, order);
  }
...
export function getModelSlot(model: Pick<ModelMappingEntry, "name" | "slot">) {
  return model.slot?.trim() || "anthropic/claude-claude-...";
}
    ...
    providers: config.providers.map((provider) => ({
      ...
      models: provider.models.map((model, index) => {
        return {
          ...
          slot: normalizeModelSlot(model.slot, model.name, provider.name || provider.id || "provider", index + 1),
