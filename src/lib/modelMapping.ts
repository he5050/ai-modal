...
function isAutoGeneratedSlot(slot: string, modelName: string) {
  ...
  return [
    ...
    // 识别自动生成的 custom 槽位
    new RegExp("^anthropic/claude-custom-\\d+$", "i"),
    new RegExp("^claude-custom-\\d+$", "i"),
  ].some((pattern) => pattern.test(slot));
}

function normalizeExplicitSlot(slot: string) {
  const trimmed = slot.trim();
  // 如果槽位已经在 DEFAULT_CLAUDE_SLOTS 中，直接返回标准格式
  const normalizedSlot = trimmed.toLowerCase().replace(/^anthropic\//i, "");
  const matchedSlot = DEFAULT_CLAUDE_SLOTS.find(
    (s) => s.toLowerCase().replace(/^anthropic\//i, "") === normalizedSlot
  );
  if (matchedSlot) {
    return matchedSlot;
  }
  // 否则规范化槽位名称
  const safe = normalizedSlot
    .split("")
    .map((char) => (/^[a-zA-Z0-9_.-]$/.test(char) ? char : "-"))
    .join("");
  ...
  return {
    providers: config.providers.map((provider) => ({
      ...
      models: provider.models.map((model) => {
        ...
        // 不再自动分配槽位，只保留用户手动设置的槽位
        const normalizedSlots = getModelSlots(model)
          .filter((s) => s && !isAutoGeneratedSlot(s, model.name))
          .map((s) => normalizeExplicitSlot(s));
