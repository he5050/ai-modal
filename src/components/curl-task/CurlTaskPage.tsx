...
import { loadCurlTasks, deleteCurlTask, executeCurlRaw, saveCurlTask } from "@/api"
...
export default function CurlTaskPage({ onOpenDetail }: CurlTaskPageProps) {
	...
	async function handleRun(task: CurlTask) {
		try {
			...
			const result = await executeCurlRaw(task.curl)
							...
							<div
								...
								<div className='flex items-center justify-between px-4 py-3'>
									<div className='min-w-0 flex-1'>
										<div className='flex items-center gap-2'>
											<h3 className='truncate text-sm font-medium text-text-heading'>{task.label}</h3>
										...
										<div className='mt-1 flex items-center gap-3 text-[11px] text-text-muted'>
											<span
												...
												title={`更新于 ${task.updatedAt ? new Date(task.updatedAt).toLocaleString("zh-CN") : ""}`}>
											...
											{task.lastRunAt && (
												<span
													className='flex items-center gap-1'
													title={`运行于 ${new Date(task.lastRunAt).toLocaleString("zh-CN")}`}>
									...
									<div className='ml-3 flex shrink-0 items-center gap-1'>
										...
										{task.lastResult && (
											<button
												onClick={() => setShowResultId(showResultId === task.id ? null : task.id)}
